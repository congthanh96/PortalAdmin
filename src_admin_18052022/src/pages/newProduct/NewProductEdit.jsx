...
import InfoProduct from '../../components/upload/dynamicForm/InfoProductEdit'
...
export default function NewProductEdit() {
  ...
  const infoProductTrans = location.state?.infoProduct
  ...
  const handleSubmitCreateProduct = async () => {
    ...
    if (infoProduct.categoryId === '' || (infoProduct[0] && infoProduct[0].categoryId === '')) {
      alert('Chọn danh mục sản phẩm để tiếp tục!')
    ...
    try {
      setLoading(true)
      if (infoImageProduct === undefined) {
        // chạy trường hợp k sửa ảnh
        await EditProductNotImage()
        setLoading(false)
      } else {
        /// B1. upload ảnh
        const uploadImageProduct = () => {
          ...
          axios({
            ...
            .then((response) => {
              ...
              console.log('nhận response', infoProduct)
              EditProductImage()
            })
        }
        ...
        /// B2. upload thông tin vừa sửa.

        setLoading(false)
        // chạy trường hợp sửa ảnh.
      }
    } catch (err) {
      console.log(err)
      setLoading(false)
  ...
  const EditProductNotImage = async () => {
    console.log(infoProduct[0])

    try {
      axios(`${API}/Newee/ManagerProduct/Edit`, {
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
          Authorization: `bearer ${user}`,
        ...
        method: 'POST',
        data: JSON.stringify(infoProduct[0]),
      })
        .then((res) => {
          console.log(res)
  ...
  const EditProductImage = async () => {
    ...
    try {
      axios(`${API}/Newee/ManagerProduct/Edit`, {
        ...
        method: 'POST',
        data: JSON.stringify(infoProduct),
      })
        .then((res) => {
          console.log(res)
    ...
    <div className="newProduct">
      {isLoading || loading ? (
        ...
        <>
          ...
          <h1 className="addProductTitle">Sửa sản phẩm!</h1>
          <div className="newProduct-container">
            <div className="newProduct-info">
              <label>#1 - Sửa thông tin sản phẩm (1/2)</label>
            ...
            {/* <div className="newProduct-variants">
              <label>#3 - Sửa phân loại (3/3)</label>
