  ...
  const handleInputChange = (index, event) => {
    ...
    if (values !== undefined) {
      if (
        ...
        console.log("123", values[index].price);
        values[index][event.target.name] = event.target.value * 1;

        if (values[index].price === undefined) {
          return;
        }
        values[index].priceDiscount =
          values[index].price -
          (event.target.value * 1 * values[index].price) / 100;
      } else {
            ...
            >
              ...
              <div className="form-group col-sm-5">
                ...
                <input
                  ...
                  id="discount"
                  name="discount"
                  value={Number(inputField.discount).toString()}
                  onChange={(event) => handleInputChange(index, event)}
                />
              </div>

              <div className="form-group col-sm-5">
                <label htmlFor="Seller nhận được">Giá bán Khuyến mãi VND</label>
                <input
                  type="text"
                  className="form-control"
                  id="priceDiscount"
