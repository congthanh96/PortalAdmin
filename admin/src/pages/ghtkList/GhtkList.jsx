...
export default function GhtkList() {
  ...
  const arrayStatus = ['Cancel', 'Pending', 'Accecpt', 'Preparing', 'Shipping', 'Delivered']

  const handleTransStatus = async (idBill, status) => {
    console.log(idBill, status)
    setLoading2(idBill)
    let arrayStatusResult = []
    let arrayStatus2 = ['Cancel', 'Pending', 'Accecpt', 'Preparing', 'Shipping', 'Delivered']
    ...
    } else if (arrayStatusResult.length === 2 || index2 * 1 - index1 * 1 === 2) {
      await dispatch(changeStatusOrder(idBill, arrayStatusResult[1]))
      console.log('5')
    } else if (arrayStatusResult.length === 3 || index2 * 1 - index1 * 1 === 3) {
      console.log('6')
      await dispatch(changeStatusOrder(idBill, arrayStatusResult[1]))
      await dispatch(changeStatusOrder(idBill, arrayStatusResult[2]))
  ...
  const townsFilter = useCallback(
    (name, key, status) => {
      setCheckStatus(!checkStatus)
      const arrayStatus3 = ['Cancel', 'Pending', 'Accecpt', 'Preparing', 'Shipping', 'Delivered']
        ...
        return (
          <>
            {params.row.idShip ? (
              ...
              >
                <button className="orderListEditDev">{params.row.shipStatusText}</button>
              </Link>
            ) : (
              <>
                <select
                  className={'order form-select ' + params.row.status}
                  onClick={(value, key) => townsFilter(value, key, params.row.status)}
                  onTouchStart={(value, key) => townsFilter(value, key, params.row.status)}
                  onChange={(value, key) => townsFilter(value, key, params.row.status)}
                >
                  <option className={params.row.status}>{params.row.status}</option>
                  {downCategory.map((value, key) => {
                    return (
                      <option value={value.id} key={key} style={{ padding: 10 }}>
