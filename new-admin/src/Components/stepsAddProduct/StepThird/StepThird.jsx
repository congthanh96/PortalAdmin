import React, { useEffect, useState } from "react";
import { Collapse, Input, Form, Select } from "antd";
import { PlusSquareOutlined,MinusSquareOutlined } from "@ant-design/icons";
import "./stepThird.css";

const { Option } = Select;
const { Panel } = Collapse;
const StepThird = ({ dataForAddProduct, setDataForAddProduct }) => {
  const [form] = Form.useForm();
  const variant = {
    price: 0,
    priceSeller: 0,
    moneyReceived: 0,
    percent: 0,
    propertyName: "",
    propertyValue: "",
    count: 0,
    sku: "",
    imageFile: "",
    srcImage:null,
    weight: 0,
    tag1: false,
    tag7: false,
  };
//   const [dataForAddProduct.variants, setLstVariant] = useState([]);
  useEffect(() => {
    const values=[...dataForAddProduct.variants, variant]
   // setLstVariant(values);
    setDataForAddProduct({...dataForAddProduct,variants:values})
    //setDataForAddProduct({...dataForAddProduct,variants:dataForAddProduct.variants})
  }, []);
  const genExtra = (index) => (
   <>
   {
    index>0?<MinusSquareOutlined  style={{ fontSize: "120%", marginRight:10, color:"red",cursor:"pointer" }}
    onClick={async (event) => {
        event.stopPropagation();
        const values = [...dataForAddProduct.variants]
        values.splice(index,1)
        //setLstVariant(values)
        setDataForAddProduct({...dataForAddProduct,variants:values})
    }}
    />:<></>
   }
   
    <PlusSquareOutlined
      style={{ fontSize: "120%", color:"green", cursor:"pointer" }}
      onClick={async (event) => {
        // If you don't want click extra trigger collapse, you can prevent this:
        event.stopPropagation();
        const values=[...dataForAddProduct.variants, variant]
        //setLstVariant(values);
        setDataForAddProduct({...dataForAddProduct,variants:values})
        //setDataForAddProduct({ ...dataForAddProduct, variants: dataForAddProduct.variants });
      }}
    />
   </>
        
  );
  const handleChangeDataInput = (event, index) => {
    const values = [...dataForAddProduct.variants];
    console.log(index);
    console.log(values[index][event.target.name]);
    values.forEach((value, idx) => {
      if (idx === index) {
        values[index][event.target.name] = event.target.value;
      }
    });
    console.log(values);
   //setLstVariant(values);
   setDataForAddProduct({ ...dataForAddProduct, variants: dataForAddProduct.variants });
  };

  const handleDataSelectTag1 = (event,index)=>{
    const values = [...dataForAddProduct.variants];
    values.forEach((value, idx) => {
      if (idx === index) {
        values[index]["tag1"] = event;
      }
    });
    console.log(values);
   setDataForAddProduct({ ...dataForAddProduct, variants: dataForAddProduct.variants });
  }
  const handleDataSelectTag7 = (event,index)=>{
    const values = [...dataForAddProduct.variants];
    values.forEach((value, idx) => {
      if (idx === index) {
        values[index]["tag7"] = event;
      }
    });
    console.log(values);
   setDataForAddProduct({ ...dataForAddProduct, variants: dataForAddProduct.variants });
  }
    
  const handleUpdateImgVariant = (event,index) => {
    const [file] = event.target.files;
    const values = [...dataForAddProduct.variants];
    values.forEach((value, idx) => {
        if (idx === index) {
            values[index]["imageFile"] = file;
            values[index]["srcImage"] = URL.createObjectURL(file);
        }
      });
      console.log(values);
    //   setLstVariant(values);
      setDataForAddProduct({...dataForAddProduct,variants:values})
    // console.log(file);
    // console.log(dataForAddProduct.link);
    //setSrcImageProduct(URL.createObjectURL(file));
  };

  return (
    <div className="container-step3">
      <h3>Thêm variant cho sản phẩm:</h3>
      <Collapse accordion>
        {dataForAddProduct.variants.map((value, index) => {
          return (
            <Panel header="Nhập variant" key={index} extra={genExtra(index)}>
              <Form layout="inline" form={form}>
                <div className="container-item-step3">
                  <span className="css-label-step3">Tên phân loại *</span>
                  <Input
                    placeholder="nhập tên"
                    className="css-input-item-step3"
                    value={value.propertyName}
                    name="propertyName"
                    onChange={(event) => handleChangeDataInput(event, index)}
                  />
                </div>
                <div className="container-item-step3">
                  <span className="css-label-step3">Giá trị phân loại *</span>
                  <Input
                    placeholder="nhập giá trị"
                    className="css-input-item-step3"
                    value={value.propertyValue}
                    name="propertyValue"
                    onChange={(event) => handleChangeDataInput(event, index)}
                  />
                </div>
                <div className="container-item-step3">
                  <span className="css-label-step3">SKU *</span>
                  <Input
                    placeholder="nhập sku"
                    className="css-input-item-step3"
                    value={value.sku}
                    name="sku"
                    onChange={(event) => handleChangeDataInput(event, index)}
                  />
                </div>
                <div className="container-item-step3">
                  <span className="css-label-step3">Số lượng *</span>
                  <Input
                    placeholder="nhập số lượng"
                    className="css-input-item-step3"
                    min={0}
                    value={value.count}
                    name="count"
                    onChange={(event) => handleChangeDataInput(event, index)}
                    type="number"
                  />
                </div>
                <div className="container-item-step3">
                  <span className="css-label-step3">Giá niêm yết *</span>
                  <Input
                    placeholder="nhập giá niêm yết"
                    className="css-input-item-step3"
                    value={value.price}
                    min={0}
                    name="price"
                    suffix="VND"
                    type="number"
                    onChange={(event) => handleChangeDataInput(event, index)}
                  />
                </div>
                <div className="container-item-step3">
                  <span className="css-label-step3">Chiết khấu *</span>
                  <Input
                    placeholder="nhập chiết khấu"
                    className="css-input-item-step3"
                    min={0}
                    value={value.percent}
                    name="percent"
                    onChange={(event) => handleChangeDataInput(event, index)}
                    type="number"
                    suffix="%"
                  />
                </div>
                <div className="container-item-step3">
                  <span className="css-label">
                    Khối lượng *
                  </span>
                  <Input
                    placeholder="nhập khối lượng"
                    className="css-input-item-step3"
                    value={value.weight}
                    min={0}
                    name="weight"
                    suffix="gam"
                    type="number"
                    onChange={(event) => handleChangeDataInput(event, index)}
                  />
                </div>
                <div className="container-item-step3">
                  <span className="css-label" style={{ width: 500 }}>
                    Seller nhận được *
                  </span>
                  <Input
                    placeholder="nhập số tiền"
                    className="css-input-item-step3"
                    min={0}
                    value={value.moneyReceived}
                    name="moneyReceived"
                    suffix="VND"
                    type="number"
                    onChange={(event) => handleChangeDataInput(event, index)}
                  />
                </div>
                <div className="container-item-step3">
                  <span className="css-label">Hàng dễ vỡ *</span>
                  <Select
                    defaultValue="False"
                    className="css-input-select-step3"
                    name="tag1"
                    onSelect={(event,name) => handleDataSelectTag1(event,index,name)}
                  >
                    <Option value="true">True</Option>
                    <Option value="false">False</Option>
                  </Select>
                </div>
                
                <div className="container-item-step3">
                  <span className="css-label">Hàng nông sản *</span>
                  <Select
                    defaultValue="False"
                    className="css-input-select-step3"
                    name="tag7"
                    onSelect={(event,name) => handleDataSelectTag7(event,index,name)}
                  >
                    <Option value="true">True</Option>
                    <Option value="false">False</Option>
                  </Select>
                </div>
                <div className="container-item-step3" style={{ width: 800 }}>
                  <span className="css-label-step3">Chọn hình ảnh *</span>
                  {/* <image
                  class="custom-avt"
                  width={200}
                  src="https://gw.alipayobjects.com/zos/rmsportal/KDpgvguMpGfqaHPjicRK.svg"
                  fallback="data:image/png;base64,"
                >
                  <div class="overlay">
                    <button>CAMERA</button>
                  </div>
                </image> */}
                  <label htmlFor={"imgProduct"+index}>
                    <img
                      className="css-image-step3"
                      src={
                        value.srcImage||
                        "data:image/png;base64,"
                      }
                      alt=""
                    />
                    <input
                      type="file"
                      id={"imgProduct"+index}
                      style={{ display: "none" }}
                      onChange={(event)=>handleUpdateImgVariant(event,index)}
                    />
                  </label>
                </div>
              </Form>
            </Panel>
          );
        })}
      </Collapse>
    </div>
  );
};

export default StepThird;
